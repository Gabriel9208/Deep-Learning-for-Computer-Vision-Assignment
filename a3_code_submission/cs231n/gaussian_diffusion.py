...
class GaussianDiffusion(nn.Module):
    ...
    def predict_start_from_noise(self, x_t, t, noise):
        ...
        ####################################################################
        sigma = extract(self.sqrt_one_minus_alphas_cumprod, t, noise.shape)
        x_start  = (x_t - sigma * noise) / extract(self.sqrt_alphas_cumprod, t, noise.shape)
        ####################################################################
        return x_start
    ...
    def p_sample(self, x_t, t: int, model_kwargs={}):
        ...
        #      and sample x_{t-1}.
        ##################################################################
        predicted = self.model(x_t, t, model_kwargs) # noise or x_start
        x_start = self.predict_start_from_noise(x_t, t, predicted) if self.objective == "pred_noise" else predicted        
    ...
    def p_losses(self, x_start, model_kwargs={}):
        ...
        loss = torch.mean(torch.square(predicted - target) * loss_weight)
