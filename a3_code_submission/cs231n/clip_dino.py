...
def clip_zero_shot_classifier(clip_model, clip_preprocess, images,
    ...
    ############################################################################
    img_processed = [clip_preprocess(Image.fromarray(img)).unsqueeze(0) for img in images]
    img_embed = clip_model.encode_image(torch.cat(img_processed, dim=0).to(device))

    text_tokenized = clip.tokenize(class_texts).to(device)
    text_embed = clip_model.encode_text(text_tokenized)
    
    similarity = get_similarity_no_loop(img_embed, text_embed)
    classes = np.argmax(similarity, axis=1)
    pred_classes = [class_texts[i] for i in classes]
    ############################################################################
    #                             END OF YOUR CODE                             #
    ############################################################################
...
class CLIPImageRetriever:
    ...
    def __init__(self, clip_model, clip_preprocess, images, device):
        ...
        ############################################################################
        images_processed = [clip_preprocess(Image.fromarray(img)).unsqueeze(0) for img in images]
        images_concat = torch.concat(images_processed, dim=0).to(device)
        self.clip_model = clip_model
        self.images_encoded = clip_model.encode_image(images_concat)
        self.image_norm = np.linalg.norm(self.images_encoded)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        pass
    ...
    def retrieve(self, query: str, k: int = 2):
        ...
        ############################################################################
        device = self.images_encoded.device
        text = clip.tokenize([query]).to(device)
        text_embed = self.clip_model.encode_text(text)

        similarity = (text_embed @ self.images_encoded.T) / (self.image_norm * text_embed.norm(dim=-1, keepdim=True))
        _, top_indices = torch.topk(similarity, k)
        top_indices = top_indices[0].cpu().flatten()
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return top_indices
...
class DINOSegmentation:
    def __init__(self, device, num_classes: int, inp_dim : int = 384):
        ...
        self.model = nn.Sequential(
          nn.Linear(inp_dim, inp_dim, device=device),
          nn.BatchNorm1d(inp_dim),
          nn.ReLU(inplace=True),
          nn.Linear(inp_dim, num_classes, device=device),
        ).to(device)
        self.device = device
    ...
    def inference(self, X_test):
        ...
        pred_classes = torch.argmax(nn.functional.softmax(out, dim=-1), dim=-1)
