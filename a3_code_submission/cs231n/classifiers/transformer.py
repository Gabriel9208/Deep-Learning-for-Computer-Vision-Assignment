...
class CaptioningTransformer(nn.Module):
    ...
    def forward(self, features, captions):
        ...
        ############################################################################
        embeddings = self.embedding(captions) # (N, T, W) 
        pe_embed = self.positional_encoding(embeddings) 
        img_proj = self.visual_projection(features).unsqueeze(1) # (N, 1, W)

        att_mask = torch.tril(torch.ones(T, T))
        out = self.transformer(tgt=pe_embed, memory=img_proj, tgt_mask=att_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        img_patched_emb = self.patch_embed(x) # (N, num_patches, embed_dim)
        pe_img_patched_emb = self.positional_encoding(img_patched_emb) # (N, num_patches, embed_dim)

        out = self.transformer(pe_img_patched_emb)
        avg_pool = torch.mean(out, dim=1) # (N, embed_dim)
        logits = self.head(avg_pool)
