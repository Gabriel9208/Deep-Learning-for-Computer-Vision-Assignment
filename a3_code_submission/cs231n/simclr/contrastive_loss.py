...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        ##############################################################################
        sim_score = sim(z_k, z_k_N)
        numerator = torch.exp(sim_score / tau)
...
def sim_positive_pairs(out_left, out_right):
    ...
    # HINT: torch.linalg.norm might be helpful.                                  #
    ##############################################################################
    dot = torch.linalg.vecdot(out_left, out_right, dim=1).unsqueeze(1)
...
def compute_sim_matrix(out):
    ...
    dot = torch.matmul(out, out_T)

    norm_out = torch.linalg.norm(out, dim=1, keepdim=True)
    norm_out_T = torch.linalg.norm(out_T, dim=0, keepdim=True)

    sim_matrix = dot / (torch.matmul(norm_out, norm_out_T))
    ##############################################################################
    #                               END OF YOUR CODE                             #
    ##############################################################################
    return sim_matrix
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    sim_ij = sim_positive_pairs(out_left, out_right) 
    sim_ji = sim_positive_pairs(out_right, out_left) 
    
    similarity_numerator = torch.cat([sim_ij, sim_ji], dim=0)
    
    # Step 3: Compute the numerator value for all augmented samples.
    numerator = torch.exp(similarity_numerator / tau)
