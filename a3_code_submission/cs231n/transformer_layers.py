...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        div_term = 10000 ** -(torch.arange(0, embed_dim, 2, dtype=torch.float32).unsqueeze(0) / embed_dim)
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        q_heads = Q.view(N, S, self.n_head, -1).permute(0, 2, 1, 3)
        k_heads = K.view(N, T, self.n_head, -1).permute(0, 2, 1, 3)
        v_heads = V.view(N, T, self.n_head, -1).permute(0, 2, 1, 3)

        k_heads_transpose = k_heads.permute(0, 1, 3, 2)
        qk_heads = torch.matmul(q_heads, k_heads_transpose) / math.sqrt(self.head_dim)
        

        if attn_mask is not None:
          qk_heads = qk_heads.masked_fill(attn_mask.view(1, 1, S, T) == 0, float('-inf'))
        softmax = F.softmax(qk_heads, dim=-1)

        dropout = self.attn_drop(softmax)

        att = torch.matmul(dropout, v_heads)
        att = att.permute(0, 2, 1, 3).contiguous().view(N, S, E)
        output = self.proj(att)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return output
...
class PatchEmbedding(nn.Module):
    ...
    def forward(self, x):
        ...
        ############################################################################
        img_embed = x.reshape(N, C, H//self.patch_size, self.patch_size, W//self.patch_size, self.patch_size)
        img_embed = img_embed.permute(0, 2, 4, 1, 3, 5)
        img_embed = img_embed.reshape(N, self.num_patches, self.patch_dim)
        out = self.proj(img_embed)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return out
