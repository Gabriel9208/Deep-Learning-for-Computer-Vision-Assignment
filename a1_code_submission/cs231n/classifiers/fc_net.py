...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        affine1 = x @ self.params['W1'] + self.params['b1'] #(N, hidden)
        jacobian = (affine1 > 0)
        relu = affine1 * jacobian # (N, hidden)
        scores = relu @ self.params['W2'] + self.params['b2'] #(N, class_num)
        ...
        loss = -np.sum(log_s[range(batch_size), y]) / batch_size \
            + 0.5 * self.reg * np.sum(np.square(self.params['W1'])) \
            + 0.5 * self.reg * np.sum(np.square(self.params['W2']))

        # softmax grad
        softmax_grad = softmax.copy() #(N, class_num)
        softmax_grad[range(batch_size), y] -= 1
        softmax_grad /= batch_size

        # affine grad
        grads["W2"] = relu.T @ softmax_grad #(hidden, class_num)
        grads["W2"] += self.reg * self.params['W2']
        grads["b2"] = np.sum(softmax_grad, axis=0)

        # relu grad
        relu_grad = softmax_grad @ self.params['W2'].T # (N, class_num) (class_num, hidden)
        relu_grad = relu_grad * jacobian #(N, hidden)

        # affine grad
        grads["W1"] = x.T @ relu_grad # (input_dim, N), (N, hidden)
...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        out, cache[1] = affine_relu_forward(reshaped_x, self.params["W1"], self.params["b1"])
