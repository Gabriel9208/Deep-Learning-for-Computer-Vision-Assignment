...
def affine_bn_relu_backward(dout, cache):
    ...
    d_aff, d_gamma, d_beta  = batchnorm_backward_alt(d_bn, bn_cache)
    dx, dw, db = affine_backward(d_aff, fc_cache)

    return dx, dw, db, d_gamma, d_beta

def affine_ln_relu_forward(x, w, b, gamma, beta, bn_param):
    affine_out, fc_cache = affine_forward(x, w, b)
    bn_out, ln_cache = layernorm_forward(affine_out, gamma, beta, bn_param)
    relu_out, relu_cache = relu_forward(bn_out)
...
def affine_ln_relu_backward(dout, cache):
    fc_cache, bn_cache, relu_cache = cache
    d_ln = relu_backward(dout, relu_cache)
    d_aff, d_gamma, d_beta  = layernorm_backward(d_ln, bn_cache)
    dx, dw, db = affine_backward(d_aff, fc_cache)
