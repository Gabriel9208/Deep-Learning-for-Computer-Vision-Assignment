...
class CaptioningRNN:
    ...
    def loss(self, features, captions):
        ...
        if self.cell_type == "rnn":
          h = rnn_forward(word_emb, h, Wx, Wh, b)
        
        vocab = temporal_affine_forward(h, W_vocab, b_vocab)
        loss = temporal_softmax_loss(vocab, captions_out, mask)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
    ...
    def sample(self, features, max_length=30):
        ...
        word_emb = None
        caption_list = []
        vocab = torch.ones(N, dtype=torch.int) * self._start
        
        for t in range(max_length):
          word_emb = word_embedding_forward(vocab, W_embed)
          h = rnn_step_forward(word_emb, h, Wx, Wh, b)
          scores = temporal_affine_forward(h.view(h.shape[0], 1, h.shape[1]), W_vocab, b_vocab)
          vocab = torch.argmax(scores.view(scores.shape[0], scores.shape[2]), dim=1)
          caption_list.append(vocab)

        captions = torch.stack(caption_list, dim=1)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        return captions
