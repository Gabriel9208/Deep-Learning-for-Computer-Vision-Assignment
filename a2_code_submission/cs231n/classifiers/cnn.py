...
class ThreeLayerConvNet(object):
    ...
    def loss(self, X, y=None):
        ...
        hidden_out, hidden_cache = affine_relu_forward(flat, W2, b2)

        scores, aff_cache = affine_forward(hidden_out, W3, b3)
        ############################################################################
        #                             END OF YOUR CODE                             #
        ############################################################################
        ...
        # softmax grad
        softmax_grad = softmax.copy() 
        softmax_grad[range(N), y] -= 1
        softmax_grad /= N

        # affine grad
        d_aff, grads["W3"], grads["b3"] = affine_backward(softmax_grad, aff_cache)
        d_hidden, grads["W2"], grads["b2"] = affine_relu_backward(d_aff, hidden_cache)
        unflat = d_hidden.reshape(conv_out.shape)
        d_conv, grads["W1"], grads["b1"] = conv_relu_pool_backward(unflat, conv_cache)
